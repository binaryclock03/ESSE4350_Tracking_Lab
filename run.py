  ...
  if True:#sat.name == "SBIIR-9(PRN21)":
    ...
    if len(AOS_time)+len(LOS_time) > 0:
      ...
      for i, time_thing in enumerate(times_to_resim):
        ...
        local_resolution = tracking_resolution/10000
        local_tstart_ref = (tracking_datetime-ep2dat(sat.refepoch)) 
        ...
        if el[0] >= 9 and el[0] <= 89 and not (time_thing == 0.0):
          AOS_bool = True
        ...
        AOS_time_n += AOS_time_l
        LOS_time_n += LOS_time_l
    
    constellation_AOS_LOS.update({sat.name:[AOS_time_n, LOS_time_n]})
